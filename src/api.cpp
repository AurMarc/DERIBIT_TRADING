...
//     // addSubscriptions already handles the prefix
...
// string api::stream_market_data(const string& input) {
//     if (subscriptions.empty()) {
//         fmt::print(fg(fmt::color::red) | fmt::emphasis::bold,
//             "> No active subscriptions. Please subscribe to symbols first.\n");
//         return "";
//     }
...
// void api::process_market_update(const json& data) {
//     try {
//         if (data.contains("params") && data["params"].contains("data")) {
//             const auto& market_data = data["params"]["data"];
//             const auto& channel = data["params"]["channel"];
            
//             auto timestamp = std::chrono::system_clock::now();
//             auto time_str = utils::format_time(timestamp);

//             if (channel.get<string>().find("price_index") != string::npos) {
//                 fmt::print("[{}] {} Price: ${:.2f}\n",
//                     time_str,
//                     channel.get<string>().substr(19), // Remove "deribit_price_index."
//                     market_data["price"].get<double>());
//             }
//         }
//     } catch (const json::exception& e) {
//         fmt::print(fg(fmt::color::red), "Error processing market data: {}\n", e.what());
//     }
// }
...
string api::authorize(const string &input) {
    ...
    s >> id >> auth >> client_id >> client_secret >> flag;
    string nonce = utils::gen_random(10);

    jsonrpc j;
    j["method"] = "public/auth";
    j["params"] = {
        {"grant_type", "client_credentials"},  // Using simple client credentials auth
        {"client_id", client_id},
    ...
    // Set AUTH_SENT flag if -s flag is provided and request is valid
    if (flag == "-s" && j.dump() != "") {
        AUTH_SENT = true;
    }
...
string api::sell(const string &input) {
    ...
    vector<string> order_types = {
        "limit", "stop_limit", "take_limit", "market", 
        "stop_market", "take_market", "market_limit", "trailing_stop"
    };
